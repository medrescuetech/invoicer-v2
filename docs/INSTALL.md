...
2. Extract `InvoiceReceiptManager_2.0.12_Portable.zip` to the desired folder (for example, a OneDrive or USB folder).
3. The archive contains the `.exe` plus the full data tree (`data/`, `documents/`, `exports/`, `backups/`, `logs/`, and `config.json`).
4. Run `InvoiceReceiptManager-2.0.12.exe` from that folder. It will use the local data automatically.
...
- **Only one instance can run at a time.** If the app does not open, check the Task Manager for a lingering `InvoiceReceiptManager-2.0.12.exe` process.
